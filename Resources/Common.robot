...
Begin Choice Number Web Test
    ...
    Maximize Browser Window
    Log Suite Start Time
    Visit Jio.com And Goto           xpath=//a[@class='j-link group__link']//div[@class='j-text j-text-body-m'][normalize-space()='Choice number']
...
Take Page Screenshot     
    [Arguments]     ${Sc_Name}
    Sleep   200ms
    Run Keyword And Continue On Failure    Wait Until Keyword Succeeds    2mins    10s    Capture Page Screenshot        ${Sc_Name}.png  
...
Plan Page UI Verification
    [Arguments]                      ${title}  
    Text Verification                xpath=//section[@id="headingCategory"]//div[@class="Filter_category__2vFb8 j-text j-text-heading-xs"]   ${title}
    ...
    ${elements}=    Get WebElements  xpath=//div[@class="MoreInformation_buttonCont__1bLIb"]//button[contains(@class,'secondary')]
...
Recharge Popup Verification
    [Arguments]            ${Recharge_locator}      ${Amt_locator}     ${View_Button}   ${RURL}    ${Number}    #Only Pass Prepaid Number
    ...
    IF  ${Popup_Presense} == True
        ...
        Enter Number and Proceed                  //div[contains(@class,'j-modal j-modal-visible j-modal-closable')]//input[@type='tel']  ${Number}   //div[contains(@class,'j-modal j-modal-visible j-modal-closable')]//button[@aria-label="button Continue"] 
    ...
    ELSE
        ...
        Enter Number and Proceed                  //div[contains(@class,'j-modal j-modal-visible j-modal-closable')]//input[@type='tel']    ${Number}   //div[contains(@class,'j-modal j-modal-visible j-modal-closable')]//button[@aria-label="button Continue"] 
        Sleep                                      3s
        Wait Until Keyword Succeeds    1mins    30s    Wait For And Verify Element               xpath=(//div[normalize-space()='Payment'][1]) | (//div[@class="css-13ado4o"])
...
Check Window count and Validation
    ...
    IF  ${len} >= 2
        Switch Window                               NEW 
        Sleep  500ms
        Run Keyword And Continue On Failure                 URL Validation                              ${FAQ-URL}        #!Hyperlink URL validation error will be ignored, can be removed if not required
        Take Page Screenshot                             ${Sc_Name}
        Sleep  100ms
        Close Window
        Switch Window                               MAIN 
